...
async def on_member_join(member):
    ...
    channel = client.get_channel(681262467229155476)
    await channel.send(f'----->Welcome to **From Russia With Love** {member.mention}!\nFor information on me (Kolbaser) do `k kolbaser`')

@client.event
async def on_member_remove(member):
    server = client.get_guild(681262467229155476)
    channel = client.get_channel(687297854011867139)
    await channel.send(f'{member} has left, farewell!')
...
async def cmds(ctx):
    ...
    embed.add_field(name='`k dm [@user] [msg]`', value='[UNAVAILAVBLE]', inline=False)
...
#    await ctx.channel.purge(limit=1)
#
#    to_channel = f"Dm sent, Thank you for using `Kolbaser's mail service` Пейте водку и хорошего дня "
...
#    await ctx.channel.send(to_channel)
...
async def on_message(message):
    ...
    if message.content.lower() == 'addy the virgin':
...
async def on_member_update(before, after):
    if before.nick != after.nick:
        ...
        with open(PATH + "\stuff.json", 'r') as f:
            ac = json.load(f)
            for key in ac:
...
@client.command()
@commands.has_any_role("Overlord", "Head Of KGB", "KGB", "top shotta")
...
@client.command()
@commands.has_any_role("Overlord", "KGB")
...
@client.command(pass_context=True)
@commands.has_any_role("Overlord", "Head Of KGB", "top shotta")
...
async def log(ctx):
    ...
    with open(PATH + "\stuff.json", 'w') as f:

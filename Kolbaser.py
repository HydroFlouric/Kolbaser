...
#BW: https://discord.com/invite/WdXs3ER
...
@client.event
async def on_ready():
    await client.change_presence(status=discord.Status.online, activity=discord.Game('Watching over all you noobs [k kolbaser]'))
    print("Bot is ready, blyat")
    print(f'Kolbaser is in {len(client.guilds)} servers')


players = {}
PATH = r"C:\Users\Addy\Desktop\F Files\Nocosteyhed\Private\F8\HYDROFLOURIC\Kolbaser[5] 2020"
...
async def cmds(ctx):
    ...
    embed.add_field(name='`k suggest`', value='Suggestion command', inline=False)
...
async def kolbaser(ctx):
    ...
    embed.add_field(name='Commands for public use', value='`k cmds,     [n/a]`', inline=False)
    embed.add_field(name='Kolbaser Admin Commands', value='`k admin`', inline=False)
    embed.add_field(name='Kolbaser Donation Information', value='`k donate[UNAVAILABLE]`', inline=False)
    embed.add_field(name='Kolbaser support server link:', value='`https://discord.gg/vmH3JCW`', inline=False)
...
def get_kolbasa():
    my_path3 = r'C:\Users\Addy\Pictures\Kolbasa\png'
...
def get_meme_image():
    my_path = r'C:\Users\Addy\Pictures\SlavMemes\SonaMemes'
...
def get_boob_image():
    my_path1 = r'C:\Users\Addy\Desktop\F Files\F9\Penumbra\Penumbra 2020 OUTDATED\Penumbra-Chive'
...
def get_russia_shit():
    mp69 = r'C:\Users\Addy\Videos\Russia shit'
...
def get_video():
    my_path5 = r'C:\Users\Addy\Videos\funny vids'
...
@client.command(pass_context=True)
@commands.has_any_role("Overlord", "KGB", "Head of KGB", "KGB Тайная Полиция")
...
async def on_message(message):
    ...
    if message.content.lower() == 'whore':
        await message.channel.send(f"You whore....you're a whore, and your mother was a whore, and her mother and your father used to turn tricks at a gas station...because he was a whore too! {message.author.mention}")

    if message.content.lower() == 'khelp':
        await message.channel.send(f"{message.author.mention} It's *k cmds*, dumbass!")

    if 'addy' in message.content:
        await message.channel.purge(limit=1)

    if 'virgin' in message.content:
        await message.channel.purge(limit=1)

    if 'addie' in message.content:
        await message.channel.purge(limit=1)

    if 'vergen' in message.content:
        await message.channel.purge(limit=1)
...
async def on_member_update(before, after):
    ...
    if before.nick != after.nick:
        PATH = r"C:\Users\Addy\Desktop\F Files\Nocosteyhed\Private\F8\HYDROFLOURIC\Kolbaser[5] 2020"
        with open(PATH + "\stuff2-5.json", 'r') as f:
            ac = json.load(f)
            for key in ac:
        ...
        PATH = r"C:\Users\Addy\Desktop\F Files\Nocosteyhed\Private\F8\HYDROFLOURIC\Kolbaser[5] 2020"
        with open(PATH + "\stuff4.json", 'r') as f:
            ar = json.load(f)
            for key in ar:
                if int(key) == after.guild.id:
                    for key in ar:

                        channel = client.get_channel(ar[key])
                        embed = discord.Embed(
                        colour = 0xdd16d9
                        )
                        embed.set_author(icon_url=after.avatar_url, name=f"{after}")
                        embed.set_thumbnail(url=after.avatar_url)
                        embed.add_field(name=f"Past Roles/{len(before.roles)}:", value=" ".join([role.mention for role in before.roles[1:]]), inline=False)
                        embed.add_field(name=f"Current Roles/{len(after.roles)}:", value=" ".join([role.mention for role in after.roles[1:]]), inline=False)
...
#                            channel = client.get_channel(uv[key])
...
async def on_member_join(member):
    ...
    with open(PATH + "\mjl.json", 'r') as f:
        guildInfo = json.load(f)
        for key in guildInfo:
            if int(key) == member.guild.id:
                for key in guildInfo:

                    channel = client.get_channel(guildInfo[key])
                    embed = discord.Embed(
                    colour = 0xdd16d9,
                    title = '***Member Joined!***',
                    ...
                    embed.add_field(name='Account Creation Date:', value=f'{member.created_at.strftime("%A, %B %#d, %Y, %H:%M UTC")}', inline=False)
                    await channel.send(embed=embed)
                    if get(member.guild.roles, name="1 poloski"):
                        channel = client.get_channel(751329457305813012)
                        ...
                        print(f'{member} joined a different server')
        ...
        for key in guildInfo:
            if int(key) == member.guild.id:
                for key in guildInfo:
                    ...
                    channelmj = client.get_channel(guildInfo[key])
                    embed = discord.Embed(
                    colour = 0xdd16d9,
                    title = '***Member Left!***',
                    )
                    embed.set_thumbnail(url=member.avatar_url)
                    embed.add_field(name=f'{member} left ***{member.guild}***', value='До свидания')
                    await channelmj.send(embed=embed)
...
@client.command()
@commands.has_any_role("Overlord", "Head Of KGB", "KGB Тайная Полиция", "KGB")
async def KGB(ctx):
    embed = discord.Embed(
    title = '***KGB Commands***',
    colour = 0xdd16d9
    ...
    embed.add_field(name='`k purge`', value='Clears int amount of messages', inline=False)
    embed.add_field(name='`k kick`', value='Kicks a user\nExample: `k kick [@user] [reason]`', inline=False)
    embed.add_field(name='`k tempmute`', value='Mutes a user for a specified amount of time\nExample: `k tempmute [@user] [amount(in seconds)] [reason]`', inline=False)
    embed.add_field(name='`k unmute`', value='Unmutes a user\nExample: `k unmute [@user]`', inline=False)
...
async def purge(ctx, number: int):
    ...
    embed = discord.Embed(
    title = '***Kolbaser***\n*Audit Log*',
    description = f"{ctx.author.mention} purged {number} message(s)\nServer: **{ctx.guild}**\nChannel: **{ctx.channel}**\nAt {time.strftime('%H:%M:%p')}",
    colour = 0xdd16d9
    )
    await channel.send(embed=embed)
...
async def tempmute(ctx, member: discord.Member, mute_time: int,* ,reason: str):
    if not member:
        await ctx.send("You need to specify who you want to mute, blyat!")
        return
...
async def execute(ctx, member: discord.Member,* ,reason: str):
    if not member:
        await ctx.send("Please specify a member")
        return
    await member.ban()
    embed = discord.Embed(
        title = '***Kolbaser***\n*Audit Log*',
        description = f"{ctx.author.mention} **executed** {member.mention}\nServer: **{ctx.guild}**\nChannel: **{ctx.channel}**\nReason: {reason}\nAt {time.strftime('%H:%M:%p')}",
        colour = 0xdd16d9
        )
    channel = client.get_channel(723701242571784194)
    await channel.send(embed=embed)
...
#################################################################################
#Admin Commands [intended for other servers]:
...
async def admin(ctx):
    ...
    embed.add_field(name='`k logs`', value='Log Menu', inline=False)
    embed.add_field(name='`k nuke`', value='Clears *many* messages in a channel', inline=False)
    embed.add_field(name='`k mute`', value='Mutes a user for a specified amount of time[mutes in HOURS(h)]\nExample: `k mute [@user] [int in HOURS] [reason]`', inline=False)
    embed.add_field(name='`k permamute`', value='Mutes a user for an indefinte about of time\nExample: `k permamute [@user] [reason]`', inline=False)
    embed.add_field(name='`k nomute`', value='Unmutes a user\nExample: `k nomute [@user] [reason]`', inline=False)
    embed.add_field(name='`k ban`', value='Bans a user\nExample: `k ban [@user] [reason]`', inline=False)
    embed.add_field(name='`k unban`', value='Unbans a user\nExample: `k unban [@user]`', inline=False)
    await ctx.send(embed=embed)
...
async def logs(ctx):
    ...
    embed.add_field(name='`k nlog`', value='Logs member nickname changes to a given channel', inline=False)
    embed.add_field(name='`k mlog`', value='Logs member joins/leaves to a given channel', inline=False)
    embed.add_field(name='`k rlog`', value='Logs member role updates to a given channel', inline=False)
    embed.add_field(name='`k mclog`', value='Logs moderation commands to a given channel[NOT OPERATIONAL YET]', inline=False)
    await ctx.send(embed=embed)
...
async def nlog(ctx):
    ...
    with open(PATH + "\stuff2-5.json", 'w') as f:
        json.dump(ac, f, indent=4)
    await ctx.send("Member nickname updates will now be logged to this channel")

@client.command()
@has_permissions(administrator=True)
async def unnlog(ctx):
    with open(PATH + "\stuff2-5.json", 'r') as f:
...
async def mlog(ctx):
    ...
    guildInfo[str(ctx.guild.id)] = ctx.channel.id #sets channel to send message to as the channel the command was sent to

    with open(PATH + "\mjl.json", "w") as f:
        json.dump(guildInfo, f, indent=4)
    await ctx.send("Member joins/leaves will now be logged to this channel")

@client.command()
@has_permissions(administrator=True)
async def unmlog(ctx):
    with open(PATH + "\mjl.json", 'r') as f:
        prefixes = json.load(f)
    ...
    with open(PATH + "\mjl.json", 'w') as f:
        json.dump(prefixes, f, indent=4)
    await ctx.send("Member joins/leaves will no longer be sent to this channel")
...
async def rlog(ctx):
    ...
    ar[str(ctx.guild.id)] = ctx.channel.id

    with open(PATH + "\stuff4.json", 'w') as f:
        json.dump(ar, f, indent=4)
    await ctx.send("Member role updates will now be logged to this channel")

@client.command()
@has_permissions(administrator=True)
async def unrlog(ctx):
    with open(PATH + "\stuff4.json", 'r') as f:
        prefixes = json.load(f)
    ...
    with open(PATH + "\stuff4.json", 'w') as f:
        json.dump(prefixes, f, indent=4)
    await ctx.send("Member role updates will no longer be sent in this channel")

@client.command()
@has_permissions(administrator=True)
async def uvlog(ctx):
    with open(PATH + "\edits.json", 'w') as f:
        prefixes = json.load(f)
    ...
    with open(PATH + "\edits.json", 'w') as f:
        json.dump(prefixes, f, indent=4)
    await ctx.send("Member avatar updates will now be logged to this channel")

@client.command()
@has_permissions(administrator=True)
async def unuvlog(ctx):
    with open(PATH + "\edits.json", 'w') as f:
        prefixes = json.load(f)
    ...
    with open(PATH + "\edits.json", 'w') as f:
        json.dump(prefixes, f, indent=4)
    await ctx.send("Member avatar updates will no longer be sent to this channel")

@client.command()
@has_permissions(administrator=True)
async def mclog(ctx):
    with open(PATH + "\mod.json", 'r') as f:
        mdcmd = json.load(f)

    mdcmd[str(ctx.guild.id)] = ctx.channel

    with open(PATH + "\mod.json", 'w') as f:
        json.dump(mdcmd, f, indent=4)
    await ctx.send("Moderation commands will now be logged to this channel")

@client.command()
@has_permissions(administrator=True)
async def unmclog(ctx):
    with open(PATH + "\mod.json", 'r') as f:
        prefixes = json.load(f)
    ...
    with open(PATH + "\mod.json", 'w') as f:
        json.dump(prefixes, f, indent=4)
    await ctx.send("Moderation commands will no longer be sent in this channel")
...
@client.command()
@has_permissions(administrator=True)
async def mute(ctx, member: discord.Member, mt: int,* ,reason=None):
    mute_var = mt * 3600
    role = discord.utils.get(ctx.guild.roles, name="Muted")
    await member.add_roles(role)
    await ctx.send(f"{member.mention} has been muted for {mt} hours by {ctx.author.mention} for {reason}")
...
@client.command()
@has_permissions(administrator=True)
async def nomute(ctx, member: discord.Member,* ,reason=None):
    role = discord.utils.get(ctx.guild.roles, name="Muted")
    await member.remove_roles(role)
...
async def ban(ctx, member: discord.Member,* ,reason: str):
    ...
    embed.add_field(name=f"Moderator: {ctx.author}\nReason: {reason}", value=f"{time.strftime('%H:%M %p')}")
...
################################################################################
#Special commands for Overlord and KGB Тайная Полиция
...
client.run('NjkwMzA1NTIzOTAzNzU4NjEy.XnPfHg.sogN1ExGpysMwu_2b6QE35D62po')
